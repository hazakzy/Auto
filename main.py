...
ENABLE_VOLATILITY_FILTER = False # Replaces ADX — skips entries when price not moving ENABLE_TRADE_LOGGING = False # Log every trade entry/exit to CSV for analysis
...
# ── Feature settings ──
VOLATILITY_CANDLES = 5 VOLATILITY_MIN_PCT = 0.8 STOP_LOSS_PCT = 4.0
DUAL_TF_INTERVAL = "15"
LOG_FILE = "/tmp/gkc_trades.csv"
# number of candles to measure price range
# skip entry if price range < 0.8% over last 5 candles
# ─── BOT MODE ─────────────────────────────────────────────────────────────────
# "trading" # "paused" BOT_MODE = "trading"
→ signals + orders + community alerts
# "signal_only" → community alerts only — no orders placed
→ completely silent — nothing fires
# ─── STATE ────────────────────────────────────────────────────────────────────
last_signal = {}
entry_price = {}
peak_profit = {}
...
print(f"[RETRACE] {symbol} peak={round(peak,3)}% — waiting for {MIN_PROFIT_TO_TRACK}%
return False
...
f" Not yet confirmed — bot waiting for candle close",
private=True
)
# ═══════════════════════════════════════════════════════════════════════════════
...
if not passes:
send_telegram(
f" <b>NEW ENTRY SKIPPED — LOW VOLATILITY</b>\n"
f"Symbol: {symbol}\n"
...
loss_pct = ((ep - price) / ep * 100 * lev) if sig == "buy" else ((price - ep) / ep if loss_pct >= STOP_LOSS_PCT:
...
print(f"[DUAL TF] {symbol} | 15m {round(fast_15m,4)}/{round(slow_15m,4)} | agrees={ag
if not agrees:
send_telegram(
f" <b>NEW ENTRY SKIPPED — DUAL TF</b>\n"
f"Symbol: {symbol}\n"
...
print(f"[PARTIAL] {symbol} closed {int(pct*100)}% ({close_qty} of {size}) | {
time.sleep(1)
update_daily_pnl(symbol)
send_telegram(
f" <b>EARLY WARNING — PARTIAL CLOSE</b>\n"
f"Symbol: {symbol}\n"
...
return
precision = get_qty_precision(symbol)
qty side = round((get_trade_usdt(symbol) * get_leverage(symbol)) / price, precision)
...
status = "PAUSED" if cfg.get("paused") else f"${cfg['trade_usdt']} x {cfg['leverage']
print(f" {s}: {status}")
print("FEATURE FLAGS:")
...
print(f"[SIGNAL] {symbol} | current={signal} | previous={prev} | mode={mo
...
if highs and not adx_allows_new_entry(symbol, highs, lows, print(f"[ADX] {symbol} ranging — position closed, no new entr
continue
# ── FIX #3: Dual TF only gates NEW entries, never exits
if not dual_tf_allows_new_entry(symbol, signal):
print(f"[DUAL TF] {symbol} disagrees — position closed, no ne
continue
...
print(f"[SIGNAL ONLY] {symbol} {signal.upper()} — alerting send_signal_only_alert(symbol, signal, price)
last_signal[symbol] = signal
bot_status["last_signal"] = last_signal.copy()
commun
else:
...
send_telegram(" <b>BOT MODE: PAUSED</b>\nNo signals, no orders", private=True)
