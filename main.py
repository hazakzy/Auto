...
def get_candles(symbol):
    ...
    for attempt in range(3):
        ...
        try:
            ...
            r = requests.get(
                f"{BASE_URL_PUBLIC}/v5/market/kline",
                params={
                    "category": "linear",
                    "symbol": symbol,
                    "interval": INTERVAL,
...
def get_price(symbol):
    ...
    for attempt in range(3):
        ...
        try:
            ...
            r = requests.get(
                f"{BASE_URL_PUBLIC}/v5/market/tickers",
                params={
                    "category": "linear",
                    "symbol": symbol
                },
...
def get_qty_precision(symbol):
    ...
    for attempt in range(3):
        ...
        try:
            ...
            r = requests.get(
                f"{BASE_URL_PUBLIC}/v5/market/instruments-info",
                params={
                    "category": "linear",
                    "symbol": symbol
                },
...
def update_daily_pnl(symbol):
    ...
    try:
        ...
        r = requests.get(
            f"{BASE_URL_PRIVATE}/v5/execution/list",
            headers=headers,
            params=params,
            timeout=10
        )
...
def set_leverage(symbol):
    ...
    r = requests.post(
        f"{BASE_URL_PRIVATE}/v5/position/set-leverage",
        headers=headers,
        json=body,
        timeout=10
    )
    ...
    if result.get("retCode") not in [0, 110043]:
        ...
        print(f"[WARN] Leverage response: {result}")
...
def close_position(symbol):
    ...
    for pos in r.json().get("result", {}).get("list", []):
        ...
        if size > 0:
            ...
            r2 = requests.post(
                f"{BASE_URL_PRIVATE}/v5/order/create",
                headers=headers,
                json=body,
                timeout=10
            )
        ...
        try:
            ...
            for symbol in SYMBOLS:
                ...
                if signal and signal != prev:
                    ...
                    print(
                        f"[ORDER] "
                        f"{symbol} {signal.upper()} confirmed"
                    )
